...
sitemap :site do
  ...
  Comfy::Blog::Post.published.each do |post|
    url post.url, last_mod: post.updated_at, change_freq: "daily", priority: 1
  end

  Comfy::Cms::Site.first.pages.root.children.published.each do |page|
    url page.url(relative: true), last_mod: page.updated_at, change_freq: "monthly", priority: 0.5
  end
end
